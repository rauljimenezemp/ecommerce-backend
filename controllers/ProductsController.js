...
const createProduct = (req, res) => {
  ...
  return ProductsModel.create(body)
    ...
    .catch(err => {
      res
        .status(400)
        .send({message: 'Error al crear un producto!', error: err.message});
      });
};
...
const updateProduct = (req, res) => {
  ...
  return ProductsModel.findByIdAndUpdate(id, body, { new: true })
    .then(updatedProduct => {
      res.status(200).send({ message: 'Producto actualizado!', updatedProduct});
    })
    .catch(err => {
      res
        .status(400)
        .send({message: 'Error actualizando producto!', error: err.message});
      });
};
